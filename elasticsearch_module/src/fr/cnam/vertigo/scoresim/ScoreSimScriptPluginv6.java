 ...
 *
 */
public class ScoreSimScriptPluginv6 extends Plugin implements ScriptPlugin {
		...
		public <T> T compile(String scriptName, String scriptSource, ScriptContext<T> context, Map<String, String> params) {
			if (context.equals(ScoreScript.CONTEXT) == false) {
				throw new IllegalArgumentException(
						getType() + " scripts cannot be used for context [" + context + "]");
			}
					...
					return new ScoreScript.LeafFactory() {
	...
	
							{
								try {
									...
									throw new IllegalArgumentException(
											getType() + "> scripts cannot be used for context [" + context + "] "+e.getMessage());
							...
							public ScoreScript newInstance(LeafReaderContext context) throws IOException {
								return new ScoreScript(p, lookup, context) {
									...
									@Override
									public double execute(ExplanationHolder arg0) {
										if(d == null)
